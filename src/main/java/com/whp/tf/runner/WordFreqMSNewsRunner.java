...
import com.whp.tf.mapper.WordFreqMSNewsMapper;
import com.whp.tf.reducer.WordFreqReducer;

public class WordFreqMSNewsRunner extends Configured implements Tool {
	...
	public int run(String[] arg0) throws Exception {
		...
		TableMapReduceUtil.initTableMapperJob(HBASE_DATA_SOURCE, scan, WordFreqMSNewsMapper.class, Text.class, IntWritable.class, job);
		
		//设置reduce输出路径
		FileOutputFormat.setOutputPath(job, new Path(arg0[0]));
		
		//设置mapper, combiner, reducer
		job.setMapperClass(WordFreqMSNewsMapper.class);
