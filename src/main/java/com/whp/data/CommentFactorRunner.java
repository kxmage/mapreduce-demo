...
import com.whp.data.mapper.CommentFactorMapper;
import com.whp.data.reducer.CommentFactorReducer;

public class CommentFactorRunner extends Configured implements Tool {

	private static final String HBASE_DATA_SOURCE = "NewsSentiments";
	...
	public int run(String[] args) throws Exception {
		...
		TableMapReduceUtil.initTableMapperJob(HBASE_DATA_SOURCE, scan, CommentFactorMapper.class, Text.class, IntWritable.class, job);
		
		//设置reduce输出路径
		FileOutputFormat.setOutputPath(job, new Path(args[0]));
		
		//设置mapper, combiner, reducer
		job.setMapperClass(CommentFactorMapper.class);
		job.setReducerClass(CommentFactorReducer.class);
	...
	public static void main(String[] args) throws Exception {
		System.exit(ToolRunner.run(new CommentFactorRunner(), args));
	}
}
