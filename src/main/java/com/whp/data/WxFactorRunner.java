...
import com.whp.data.mapper.WxFactorMapper;
import com.whp.data.reducer.WxFactorReducer;

public class WxFactorRunner extends Configured implements Tool {

	private static final String HBASE_DATA_SOURCE = "puc_weixin";
	...
	public int run(String[] args) throws Exception {
		...
		TableMapReduceUtil.initTableMapperJob(HBASE_DATA_SOURCE, scan, WxFactorMapper.class, Text.class, IntWritable.class, job);
		
		//设置reduce输出路径
		FileOutputFormat.setOutputPath(job, new Path(args[0]));
		
		//设置mapper, combiner, reducer
		job.setMapperClass(WxFactorMapper.class);
		job.setCombinerClass(WxFactorReducer.class);
		job.setReducerClass(WxFactorReducer.class);
	...
	public static void main(String[] args) throws Exception {
		System.exit(ToolRunner.run(new WxFactorRunner(), args));
	}
}
