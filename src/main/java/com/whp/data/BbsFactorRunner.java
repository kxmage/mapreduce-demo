...
import com.whp.data.mapper.BbsFactorMapper;
import com.whp.data.reducer.BbsFactorReducer;

public class BbsFactorRunner extends Configured implements Tool {

	private static final String HBASE_DATA_SOURCE = "puc_bbs";
	...
	public int run(String[] args) throws Exception {
		...
		TableMapReduceUtil.initTableMapperJob(HBASE_DATA_SOURCE, scan, BbsFactorMapper.class, Text.class, IntWritable.class, job);
		
		//设置reduce输出路径
		FileOutputFormat.setOutputPath(job, new Path(args[0]));
		
		//设置mapper, combiner, reducer
		job.setMapperClass(BbsFactorMapper.class);
		job.setCombinerClass(BbsFactorReducer.class);
		job.setReducerClass(BbsFactorReducer.class);
	...
	public static void main(String[] args) throws Exception {
		System.exit(ToolRunner.run(new BbsFactorRunner(), args));
	}
}
