...
import com.whp.data.mapper.MSNewsFactorMapper;
import com.whp.data.reducer.MSNewsFactorReducer;

public class MSNewsFactorRunner extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		TableMapReduceUtil.initTableMapperJob(HBASE_DATA_SOURCE, scan, MSNewsFactorMapper.class, Text.class, Text.class, job);
		
		//设置reduce输出路径
		FileOutputFormat.setOutputPath(job, new Path(args[0]));
		
		//设置mapper, combiner, reducer
		job.setMapperClass(MSNewsFactorMapper.class);
//		job.setCombinerClass(MSNewsFactorReducer.class);
		job.setReducerClass(MSNewsFactorReducer.class);
	...
	public static void main(String[] args) throws Exception {
		System.exit(ToolRunner.run(new MSNewsFactorRunner(), args));
	}
}
